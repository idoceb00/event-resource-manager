...
export const mockReservationService: ReservationService = {
	...
	async getReservationsForEvent(eventId: string): Promise<ReservationWithNames[]> {
		return reservations.filter((reservation) => reservation.eventId === eventId).map(withNames);
	},

	async hasConflict(reservationId: string): Promise<boolean> {
		return reservations.find((reservation) => reservation.id === reservationId)?.hasConflict ?? false;
	},
