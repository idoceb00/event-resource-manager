...
<div class="p-6">
	<div class="mb-6">
		...
		{#if conflictCount > 0}
			<div class="bg-red-50 border border-red-200 rounded p-3 flex items-start gap-3">
	...
	<DataTable
		...
		variant="gray"
		rowClass={(row: ReservationWithNames) => (row.hasConflict ? 'bg-red-50' : 'hover:bg-gray-50')}
		columns={[
...
{#snippet eventCell(row: ReservationWithNames)}
	<a href={localizeHref(`/events/${row.eventId}`)} class="text-blue-600 hover:underline">
		{row.eventName}
	</a>
{/snippet}
...
{#snippet startCell(row: ReservationWithNames)} {formatDate(row.startDate, locale)} {/snippet}

{#snippet endCell(row: ReservationWithNames)} {formatDate(row.endDate, locale)} {/snippet}
