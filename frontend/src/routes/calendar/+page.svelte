<script lang="ts">
	...
	import { buildMonthGrid, getEventsForDay, initialCalendarMonth, isSameMonth } from '$lib/utils/calendar';
	...
	const weeks = $derived(buildMonthGrid(currentDate.getFullYear(), currentDate.getMonth()));
...
<div class="p-6 max-w-7xl">
	<div class="flex items-center justify-between mb-6">
		...
		<div class="flex items-center gap-3">
			<Button variant="icon" onclick={previousMonth} aria-label={m.calendar_previous_month()}>
				<ChevronLeft class="w-5 h-5" />
			</Button>
			<div class="text-sm font-medium min-w-[140px] text-center">
				{formatMonthYear(currentDate, locale)}
			</div>
			<Button variant="icon" onclick={nextMonth} aria-label={m.calendar_next_month()}>
	...
	<div class="bg-white border border-neutral-200 rounded overflow-hidden">
		<div class="grid grid-cols-7 border-b border-neutral-200 bg-neutral-50">
			{#each dayNames as day}
				<div class="px-3 py-2 text-xs font-medium text-neutral-600 text-center">{day}</div>
			{/each}
		</div>
				...
				<div class="grid grid-cols-7 divide-x divide-neutral-200">
					{#each week as day}
						...
						<div class="min-h-[120px] p-2 {inMonth ? '' : 'bg-neutral-50'}">
							<div class="text-xs mb-2 {inMonth ? 'text-neutral-600' : 'text-neutral-400'}">
								{day.getDate()}
							</div>
							<div class="space-y-1">
								{#each dayEvents as event}
									...
									<a
										href={localizeHref(`/events/${event.id}`)}
